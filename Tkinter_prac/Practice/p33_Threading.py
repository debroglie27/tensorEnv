...
def five_seconds():
    time.sleep(5)
    my_label.config(text="Five Seconds is up!")
...
# Starts a function which can take some time
# So we use threading so that it does not freezes other things
my_button1 = Button(root, text="5 seconds", command=threading.Thread(target=five_seconds).start)
my_button1.pack(pady=20)

my_button2 = Button(root, text="Pick Random Number", command=rand_o)
